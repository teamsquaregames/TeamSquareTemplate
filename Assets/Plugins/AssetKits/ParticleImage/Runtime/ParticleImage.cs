// Version: 1.2.0 + PINPIN MODIF v4.1
...
namespace AssetKits.ParticleImage
{
	...
	public sealed class ParticleImage : MaskableGraphic
	{
		...
		//PINPIN Modif
		public bool valuesCopyiedFromOtherPI = false;

		//PINPIN Modif
		public void CopyFlyingModel(ParticleImage _model)
		{
			startSpeed = _model.startSpeed;
			lifetime   = _model.lifetime;
			startSize  = _model.startSize;

			//emission
			shape = _model.shape;
			circleRadius = _model.circleRadius;
			fitRect = _model.fitRect;
			emitOnSurface = _model.emitOnSurface;
			spreadType = _model.spreadType;
			spreadLoop = _model.spreadLoop;

			//particle
			speedOverLifetime = _model.speedOverLifetime;
			sizeOverLifetime = _model.sizeOverLifetime;
			sizeBySpeed = _model.sizeBySpeed;
			sizeSpeedRange = _model.sizeSpeedRange;

			rotationOverLifetime = _model.rotationOverLifetime;
			rotationBySpeed = _model.rotationBySpeed;
			rotationSpeedRange = _model.rotationSpeedRange;
			alignToDirection = _model.alignToDirection;

			//movement
			attractorLerp = _model.attractorLerp;
			attractorType = _model.attractorType;

			noiseEnabled = _model.noiseEnabled;
			noiseStrength = _model.noiseStrength;
			noiseFrequency = _model.noiseFrequency;
			noiseOffset = _model.noiseOffset;
			noiseViewSize = _model.noiseViewSize;

			velocityEnabled = _model.velocityEnabled;
			velocitySpace = _model.velocitySpace;
			velocityOverLifetime = _model.velocityOverLifetime;

			gravityEnabled = _model.gravityEnabled;
			gravity = _model.gravity;

			vortexEnabled = _model.vortexEnabled;
			vortexStrength = _model.vortexStrength;
		}
		...
		protected override void Awake ()
		{
			...
			if (main.children == null) //PINPIN MODIF (only the if)
				main.children = main.GetChildren();

			//PINPIN MODIF
			if (isMain && children != null)
			{
				foreach (ParticleImage item in children)
				{
					item.onParticleStop.AddListener(TrySetActiveFalse);
				}
			}
		...
		protected override void OnDestroy ()
		{
			...
			if (isMain && children != null)
			{
				foreach (ParticleImage item in children)
				{
					item.onParticleStop.RemoveListener(TrySetActiveFalse);
				}
			}
		}
		...
		public ParticleImage[] GetChildren ()
		{
			...
			//PINPIN MODIF (added param true)
			var ch = GetComponentsInChildren<ParticleImage>(true).Where(t => t != this);
		...
		private void Simulate ( float deltaTime, bool prewarming = false )
		{
			...
			//</Generate Particles>
			//PINPIN MODIF
			if (particleCount <= 0 && Application.isPlaying && isMain)
			{
				Stop(true);
			}
		...
		private void OnParticleStop ()
		{
			...
			//PINPIN MODIF
			if (Application.isPlaying && isMain)
				TrySetActiveFalse();
		}

		private void TrySetActiveFalse ()
		{
			if (!isMain || !Application.isPlaying) return;

			if (children != null)
			{
				foreach (ParticleImage item in children)
				{
					if (item.particleCount > 0 && item.gameObject.activeInHierarchy)
						return;
				}
			}
